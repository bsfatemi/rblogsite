...
# oid <- orgIndex()[short_name == nam, org_uuid]
#
...
mmdRewards <- zDT[disp_name == "NUG San Leandro" & brand_name == "PAX",
                  sum(reward_value),
...
), keyby = .(ym = floor_date(date, unit = "week"))]


## capture weeks where there was training
trainingDT <- mmdRewards[, .N, .(reward_date = floor_date(reward_date, "week"))][, .(reward_date)]
...
##
ind <- pdata4[, which(has_training_event)]

## keep only a few weeks before and after rewards event because there's only a single
## rewards event for PAX
##
pdata5 <- pdata4[(ind - 7):(ind + 7)]
pdata5[which(has_training_event):.N, is_training_period := TRUE]
pdata5[is.na(is_training_period), is_training_period := FALSE]

pdata5[is_training_period == FALSE, period := "Pre-Rewards"]
pdata5[is_training_period == TRUE, period := "Post-Rewards"]
pdata5[, period := factor(period, levels = c("Pre-Rewards", "Post-Rewards"))]

## get average for labelling plot lines
tmp <- pdata5[, .(ym = ym[1], aveWeeklySales = mean(tot_sales)), period]
tmp[, lab := scales::dollar(aveWeeklySales, accuracy = 1)]

p1 <- ggplot(pdata5, aes(ym, tot_sales)) +
  geom_line() +
  geom_point(aes(color = has_training_event), size = 5) +
  geom_hline(aes(yintercept = aveWeeklySales), tmp) +
  geom_text(aes(x = ym, y = aveWeeklySales, label = lab), tmp, vjust = -1, hjust = 0) +
  scale_x_date(date_breaks = "week", date_labels = "%b %d") +
  scale_y_continuous(labels = scales::dollar_format()) +
  xlab("") +
  ylab("") +
  ggtitle("Nug | San Leandro | PAX",
          "Sales Per Week - Before vs After Rewards Earning Day") +
