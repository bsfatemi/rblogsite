...
#                 WHERE org_uuid = '{oid}'
#                 AND facility = 'marinadelrey'
#                 AND brand_name = 'BINSKE';")
#
...
mmdRewards <- zDT[disp_name == "MMD - Marina Del Rey",
                  sum(reward_value), keyby = .(reward_date, brand_name)]


## summarize sales by day
##
pdata <- MDR[pos_category != "samples", .(
...
trainingDT <- mmdRewards[brand_name == "binske", .N,
...
tmp <- pdata5[, .(aveMonthlySales = mean(tot_sales)), period]
tmp[1, ym := as_date("2022-06-01")]
tmp[2, ym := as_date("2022-08-01")]
tmp[, lab := scales::dollar(aveMonthlySales, accuracy = 1)]

ggplot(pdata5, aes(ym, tot_sales)) +
  geom_line() +
  ...
  xlab("") +
  ylab("") +
  ggtitle("MMD | Marina Del Ray | BINKSE",
