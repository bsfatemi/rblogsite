...
zt_disp_name <- "High Times - Coalinga"
zt_brand <- "Pacific Stone"
...
mmdRewards <- zDT[disp_name == zt_disp_name & brand_name == zt_brand,
                  sum(reward_value),
...
trainingDT <- mmdRewards[, .N, .(reward_date = floor_date(reward_date, "month"))][, .(reward_date)]
...
##
pdata4[which(has_training_event)[1]:rev(which(has_training_event))[1],
...
pdata5 <- pdata4[1:rev(which(is_training_period))[1]]
...
p1 <- ggplot(pdata5, aes(ym, tot_sales)) +
  ...
  geom_text(aes(x = ym, y = aveMonthlySales, label = lab), tmp, vjust = -.8, hjust = -.01) +
  scale_x_date(date_labels = "%b/%y") +
  scale_y_continuous(labels = scales::dollar_format()) +
  xlab("") +
  ylab("") +
  ggtitle("High Times | Coalinga | Pacific Stone",
