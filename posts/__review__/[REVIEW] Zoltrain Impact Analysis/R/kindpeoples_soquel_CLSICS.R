...
trainingDT <- mmdRewards[brand_name == "CLSICS", .N,
                         .(reward_date = floor_date(reward_date, "week"))][, .(reward_date)]
...
pdata5 <- pdata4[ym < "2022-03-20" & ym > "2022-01-16"]
...
p1 <- ggplot(pdata5, aes(ym, tot_sales)) +
  ...
  geom_text(aes(x = ym, y = aveWeeklySales, label = lab), tmp, vjust = -.8, hjust = -.01) +
  scale_x_date(date_breaks = "weeks", date_labels = "%b-%d") +
  scale_y_continuous(labels = scales::dollar_format()) +
  xlab("") +
  ylab("") +
  ggtitle("Kind Peoples | Soquel | CLSICS",
          "Sales Per Week - Before vs During Rewards Earning Period") +
