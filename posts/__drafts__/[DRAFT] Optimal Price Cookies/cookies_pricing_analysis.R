...
SELECT order_time_utc,
  ...
  product_name
FROM population2
WHERE brand_name = 'COOKIES';
"
K <- dbQueryFetch(cn, qry, 10000)
dbd(cn)

K[, price_per_unit := item_list_price / product_qty]

## Tease out product weight and units of measurement
K[, product_weight := str_extract(product_name, "([0-9]+\\.)?[0-9]+ ?M?G")]
K[, product_units := str_extract(product_weight, "M?G")]
K[, product_weight := as.numeric(str_trim(str_remove(product_weight, "M?G"), "both"))]

K[is.na(price_per_unit)]
setkeyv(K, "category3")
K["FLOWER", lapply(.SD, function(x) list(ave = mean(x), std = sd(x))), 
  .SDcols = c("item_discount", "price_per_unit"), 
  by = "product_weight"]

# K[, product_name := str_replace_all(product_name, "3\\.5 G", "3\\.5G")]

K[category3 == "FLOWER" & str_detect(product_name, "GEORGIA PIE"), .N, product_name]

K[][, .N, product_name][order(-N)]


K <- h[brand_name == "COOKIES" & category3 == "EDIBLES"]
